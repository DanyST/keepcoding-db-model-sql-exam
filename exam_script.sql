create schema fleet_company_vehicles;

create table fleet_company_vehicles.brand_corporate_group(
	id SERIAL primary key,
	name varchar(80) not null,
	description varchar(300) not null
);

create table fleet_company_vehicles.brand(
	id SERIAL primary key,
	name varchar(80) not null,
	brand_corporate_group_id int not null
);

create table fleet_company_vehicles.model(
	id SERIAL primary key,
	name varchar(80) not null,
	brand_id int not null
);

create table fleet_company_vehicles.currency(
	id SERIAL primary key,
	name varchar(80) not null
);

create table fleet_company_vehicles.technical_inspection(
	id SERIAL primary key,
	kms int not null,
...
create table fleet_company_vehicles.color(
	id SERIAL primary key,
	name varchar(80) not null
);

create table fleet_company_vehicles.car(
...
create table fleet_company_vehicles.insurance_company(
	id SERIAL primary key,
	name varchar(80) not null,
	description varchar(300) not null
);

create table fleet_company_vehicles.insurance_policy(
...
alter table fleet_company_vehicles.brand add constraint fk_brand_corporate_group foreign key (brand_corporate_group_id) references fleet_company_vehicles.brand_corporate_group(id);
alter table fleet_company_vehicles.model add constraint fk_model_brand foreign key (brand_id) references fleet_company_vehicles.brand(id);
alter table fleet_company_vehicles.car add constraint fk_car_model foreign key (model_id) references fleet_company_vehicles.model(id);
alter table fleet_company_vehicles.car add constraint fk_car_color foreign key (color_id) references fleet_company_vehicles.color(id);
alter table fleet_company_vehicles.insurance_policy add constraint fk_insurance_policy_company foreign key (insurance_company_id) references fleet_company_vehicles.insurance_company(id);
alter table fleet_company_vehicles.insurance_policy add constraint fk_insurance_policy_currency foreign key (currency_id) references fleet_company_vehicles.currency(id);
alter table fleet_company_vehicles.insurance_policy add constraint fk_insurance_policy_car foreign key (car_id) references fleet_company_vehicles.car(license_plate);
alter table fleet_company_vehicles.technical_inspection add constraint fk_technical_inspection_car foreign key (car_id) references fleet_company_vehicles.car(license_plate);
alter table fleet_company_vehicles.technical_inspection add constraint fk_technical_inspection_currency foreign key (currency_id) references fleet_company_vehicles.currency(id);

CREATE TABLE fleet_company_vehicles.coches (
...
INSERT INTO fleet_company_vehicles.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- insert into brand_corporate_group
insert into fleet_company_vehicles.brand_corporate_group (name, description)
select c.grupo, '' as description
from fleet_company_vehicles.coches c
group by c.grupo
order by c.grupo;


-- insert into brand
insert into fleet_company_vehicles.brand (name, brand_corporate_group_id)
select c.marca, bcg.id brand_corporate_group_id
from fleet_company_vehicles.coches c
inner join fleet_company_vehicles.brand_corporate_group bcg on c.grupo = bcg."name"
group by c.marca, bcg.id;


-- insert into model 
insert into fleet_company_vehicles.model (name, brand_id)
select c.modelo, b.id
from fleet_company_vehicles.coches c 
inner join fleet_company_vehicles.brand b on c.marca = b."name"
inner join fleet_company_vehicles.brand_corporate_group bcg on b.brand_corporate_group_id = bcg.id
group by c.modelo, b.id
order by b.id;

--insert into color
insert into fleet_company_vehicles.color (name)
select c.color 
from fleet_company_vehicles.coches c 
group by c.color
order by c.color;
...
-- insert into car
insert into fleet_company_vehicles.car (license_plate, total_kilometers, purchase_date, model_id, color_id)
select c.matricula, c.kms_totales, c.fecha_compra, m.id, color.id
from fleet_company_vehicles.coches c
inner join fleet_company_vehicles.model m on c.modelo = m."name"
inner join fleet_company_vehicles.color color on c.color = color."name" 
group by c.matricula, c.kms_totales, c.fecha_compra,  m.id , color.id
order by c.matricula;

-- insert into currency
insert into fleet_company_vehicles.currency (name)
select c.moneda 
from fleet_company_vehicles.coches c 
group by c.moneda
order by c.moneda;

-- insert into technical inspection
insert into fleet_company_vehicles.technical_inspection (car_id, currency_id, amount, kms, inspection_date, expiration_date)
select car.license_plate, currency.id currency_id, c.importe_revision, c.kms_revision, c.fecha_revision, c.fecha_revision + INTERVAL '1 year' as expiration_date
from fleet_company_vehicles.coches c
inner join fleet_company_vehicles.currency currency on c.moneda = currency."name"
inner join fleet_company_vehicles.car car  on c.matricula  = car.license_plate 
group by car.license_plate, currency.id, c.importe_revision, c.kms_revision, c.fecha_revision
order by car.license_plate, c.fecha_revision;

-- insert insurance company
insert into fleet_company_vehicles.insurance_company (name, description)
select c.aseguradora, '' as description 
from fleet_company_vehicles.coches c 
group by c.aseguradora 
order by c.aseguradora;

-- insert insurance_policy
insert into fleet_company_vehicles.insurance_policy (car_id, insurance_company_id, policy_number, start_date, expiration_date, amount, currency_id)
select car.license_plate, insurance_company.id insurance_company_id, c.n_poliza, c.fecha_alta_seguro, c.fecha_alta_seguro + INTERVAL '1 year' as expiration_date, 50 as importe, 1 as currency_id
from fleet_company_vehicles.coches c 
inner join fleet_company_vehicles.car car on c.matricula = car.license_plate
inner join fleet_company_vehicles.insurance_company insurance_company on c.aseguradora = insurance_company."name" 
group by car.license_plate, insurance_company_id, c.n_poliza, c.fecha_alta_seguro
order by car.license_plate, c.fecha_alta_seguro;
	

-- final report with select.
select 
	distinct on (car.license_plate)
	model."name" as car_model,
	brand."name" as car_brand,
	bcg."name" as car_brand_comporate_group_name,
	car.purchase_date as purchase_date,
	car.license_plate as license_plate,
	color."name" as car_color,
	car.total_kilometers as total_kilometers,
	insurance_company."name" as insurance_company_name,
	insurance_policy.policy_number as policy_number
from fleet_company_vehicles.car car
inner join fleet_company_vehicles.model model on car.model_id = model.id 
inner join fleet_company_vehicles.brand brand on model.brand_id = brand.id
inner join fleet_company_vehicles.brand_corporate_group bcg on brand.brand_corporate_group_id = bcg.id
inner join fleet_company_vehicles.color color on car.color_id = color.id 
inner join fleet_company_vehicles.insurance_policy insurance_policy on car.license_plate = insurance_policy.car_id
inner join fleet_company_vehicles.insurance_company insurance_company on insurance_policy.insurance_company_id = insurance_company.id
group by 
	model."name",
	brand."name",
	bcg."name",
	...
	car.license_plate,
	color."name", 
	car.total_kilometers,
	insurance_company."name",
	insurance_policy.policy_number
order by 
	car.license_plate,
	insurance_policy.start_date desc;



-- delete temporal table
drop table fleet_company_vehicles.coches;
